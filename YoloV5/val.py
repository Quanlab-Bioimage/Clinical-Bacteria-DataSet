...
def run(
    ...
    feature_loss = FeatureLoss(2)
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        # if os.path.join(os.path.dirname(paths[0]), '000003_3_3.jpg') in paths:
        #     print()
        #     print()
        ...
        with dt[1]:
            ...
            # pred, featureLs = model(im, getFeature=True)  # forward
        # targets = targets.to('cpu')
        # loss_feature, loss_feature_items = feature_loss(featureLs, targets)
        # targets = targets.to(device)
        ...
        for si, pred in enumerate(preds):
            ...
            pred = torch.from_numpy(newRed).to(device)
            preds[si] = pred
...
def parse_opt():
    ...
    parser.add_argument('--verbose', action='store_true', help='report mAP by class', default=True)
